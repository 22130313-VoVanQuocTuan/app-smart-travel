...
class DestinationBloc extends Bloc<DestinationEvent, DestinationState> {
  ...
  Future<void> _onLoadAllDestinations(
    ...
    final isReady = await _ensureDestinationsLoaded(
      forceRefresh: event.forceRefresh,
    );
    if (!isReady) {
      emit(FilterDestinationError('Khong the tai danh sach dia diem'));
      return;
    }
  ...
  Future<void> _onFilterDestinationsEvent(
    ...
    final filteredDestinations = List.of(_allDestinations);
    final minPrice = event.minPrice;
    final maxPrice = event.maxPrice;
    if (minPrice != null && maxPrice != null && minPrice <= maxPrice) {
      filteredDestinations.retainWhere(
    ...
    if (minRating != null && minRating > 0) {
      filteredDestinations.retainWhere((d) => d.averageRating >= minRating);
    }
    ...
    switch (event.sortBy) {
      case 'rating':
        ...
        break;
      case 'price_low':
        filteredDestinations.sort((a, b) => a.entryFee!.compareTo(b.entryFee!));
        break;
      case 'price_high':
        filteredDestinations.sort((a, b) => b.entryFee!.compareTo(a.entryFee!));
        break;
      case 'popular':
      default:
        filteredDestinations.sort(
  ...
  Future<void> _onSearchDestinationEvent(
    ...
    var filteredDestinations = List.of(_allDestinations);
  ...
  FutureOr<void> _onAddDestinationEvent(
    ...
    result.fold(
      (failure) => emit(AddDestinationError(failure.message)),
      (success) => emit(AddDestinationSuccess(success)),
    );
    add(LoadAllDestinations(loadAll: true, forceRefresh: true));
  }
  ...
  FutureOr<void> _onUpdateDestinationEvent(
    ...
    result.fold(
      (failure) => emit(UpdateDestinationError(failure.message)),
      (success) => emit(UpdateDestinationSuccess(success)),
    );
    add(LoadAllDestinations(loadAll: true, forceRefresh: true));
  }
  ...
  FutureOr<void> _onDeleteDestinationEvent(
    ...
    result.fold(
      (failure) => emit(DeleteDestinationError(failure.message)),
      (success) => emit(DeleteDestinationSuccess(success)),
    );
    add(LoadAllDestinations(loadAll: true, forceRefresh: true));
  }
  ...
  Future<bool> _ensureDestinationsLoaded({bool forceRefresh = false}) async {
    if (!forceRefresh && _allDestinations.isNotEmpty) {
      return true;
    }
  ...
  FilterDestinationLoaded _buildLoadedState({required bool loadAll}) {
    final displayList =
        loadAll ? _allDestinations : _allDestinations.take(6).toList();
    return FilterDestinationLoaded(displayList, isLimited: !loadAll);
  }
}
